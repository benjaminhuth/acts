 ...
 public:
  ...
  LittleDrawer(SpacePointIt b, SpacePointIt e)
    ...
    auto [minRIt, maxRIt] = std::minmax_element(
        b, e, [](const auto &aa, const auto &bb) { return aa.r() < bb.r(); });
    auto [minZIt, maxZIt] = std::minmax_element(
        b, e, [](const auto &aa, const auto &bb) { return aa.z() < bb.z(); });
    ...
    for (auto it = b; it != e; ++it) {
      ...
      assert(iz < m_width && std::round(vz) >= 0);
      assert(ir < m_height && std::round(vr) >= 0);
...
struct ActsExamples::PrototracksToParsAndSeeds::SeedingImpl {
