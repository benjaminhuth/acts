...
class RootAthenaDumpWriter : public IWriter {
 public:
  struct Config {
    ...
    /// Maximum number of particles per event. The data() pointer of the
    /// internal vectors is passed to ROOT at construction time and must remain
    /// stable, so the capacity is reserved upfront and never exceeded.
    unsigned int maxParticles = 1500000;
    /// Maximum number of clusters (measurements) per event.
    unsigned int maxClusters = 1500000;
    /// Maximum number of space points per event.
    unsigned int maxSpacePoints = 1500000;
  };
