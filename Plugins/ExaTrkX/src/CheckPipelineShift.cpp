...
auto run_pipeline(std::vector<float> &data,
    ...
    auto gc = std::make_shared<Acts::TorchMetricLearning>(gcCfg, Acts::getDefaultLogger("gc", l));
    auto flt = std::make_shared<Acts::TorchEdgeClassifier>(fltCfg, Acts::getDefaultLogger("flt", l));
    auto gnn = std::make_shared<Acts::TorchEdgeClassifier>(gnnCfg, Acts::getDefaultLogger("gnn", l));
    auto trk = std::make_shared<Acts::BoostTrackBuilding>(Acts::getDefaultLogger("trk", l));

    Acts::Pipeline pipeline(gc, {flt, gnn}, trk, Acts::getDefaultLogger("pipeline", l));
...
void checkShiftInvariance(const std::string &path) {
    ...
    // Shift
    std::rotate(spacepointIDs.begin(), spacepointIDs.begin() + 1, spacepointIDs.end());
    ...
    std::transform(tracks1.begin(), tracks1.end(), sizes1.begin(), [](const auto &t){ return t.size(); });
    std::transform(tracks2.begin(), tracks2.end(), sizes2.begin(), [](const auto &t){ return t.size(); });

    std::sort(sizes1.begin(), sizes1.end());
    std::sort(sizes2.begin(), sizes2.end());

    std::cout << "Tracks 1:";
    std::copy(sizes1.begin(), sizes1.end(), std::ostream_iterator<std::size_t>(std::cout, " "));
    std::cout << std::endl;

    std::cout << "Tracks 2:";
    std::copy(sizes2.begin(), sizes2.end(), std::ostream_iterator<std::size_t>(std::cout, " "));
    std::cout << std::endl;
}
