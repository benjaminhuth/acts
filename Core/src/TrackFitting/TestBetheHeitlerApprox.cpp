...
void test_flat_cache() {
  ...
  throw_assert(cache.findApprox(1.0).has_value() &&
                   cache.findApprox(1.0)->at(0).weight == 1.0,
               "bla");
  throw_assert(cache.findApprox(2.05).has_value() &&
                   cache.findApprox(2.05)->at(0).weight == 2.0,
               "bla");
  throw_assert(cache.findApprox(2.97).has_value() &&
                   cache.findApprox(2.97)->at(0).weight == 3.0,
               "bla");
}

int main() {
#if 1
  const auto atlasBetheHeitlerApprox =
      AtlasBetheHeitlerApprox<6, 5>::loadFromFile(
          "/home/benjamin/Documents/athena/Tracking/TrkFitter/"
          "TrkGaussianSumFilter/Data/GeantSim_LT01_cdf_nC6_O5.par",
          "/home/benjamin/Documents/athena/Tracking/TrkFitter/"
          "TrkGaussianSumFilter/Data/GeantSim_GT01_cdf_nC6_O5.par");
#else
  const auto atlasBetheHeitlerApprox = AtlasBetheHeitlerApprox<6, 5>(
      bh_cdf_cmps6_order5_data, bh_cdf_cmps6_order5_data, true, true);
#endif
  ...
  auto startValue = [&]() {
    ...
    detail::normalizeWeights(ret, [](auto &a) -> double & { return a.weight; });
  ...
  auto mix_plot = [&](auto this_mixture, double thickness,
                      Kwargs kwargs = Kwargs{}) {
    auto mixture_function =
        detail::GaussianMixtureModelPDF<this_mixture.size()>{this_mixture};
    auto bh_distribution = detail::BetheHeitlerPDF{thickness};

    std::vector<double> view_bh, view_mx;
    std::transform(x.begin(), x.end(), std::back_inserter(view_bh),
                   bh_distribution);
    std::transform(x.begin(), x.end(), std::back_inserter(view_mx),
                   mixture_function);
  ...
  BetheHeitlerSimulatedAnnealingMinimizer bh(temperatures, startValue, gen,
                                             next);
  ...
  for (auto i = 0ul; i < thicknesses.size(); ++i) {
    ...
    const auto atlas_dist = boost::math::quadrature::trapezoidal(
        detail::CDFIntegrant<atlas_mixture.size()>{
            detail::GaussianMixtureModelCDF<atlas_mixture.size()>{
                atlas_mixture},
    ...
    mix_plot(atlas_mixture, thicknesses[i],
             Kwargs{{"label", fmt::format("atlas, Nc={}, dist={:.5f}",
                                          atlas_mixture.size(), atlas_dist)}});
    mix_plot(
        cdf_mixture, thicknesses[i],
        Kwargs{{"label", fmt::format("cdf, Nc={}, dist={:.5f}", NComponents,
                                     *std::ranges::min_element(cdf_history))}});
